...
static A_FpLong Frequency = 5;
static int MillisecondInSecond = 1000;
static int DimensionX = 1;
static int DimensionY = 2;
static int DimensionZ = 3;

static A_char*
GetLayerTransferModeName(
	A_long mode
)
{
	switch (mode)
	...
	AEGP_ObjectType		objectType)
{
	...
	if (!err && stream_nameZ) {		// don't bother to print blank name...
		fprintf(out, format_stringZ, indent_stringZ);
			...
			if (!err) {
				...
				case AEGP_StreamType_ThreeD:
					if (which_stream == AEGP_LayerStream_ANCHORPOINT || which_stream == AEGP_LayerStream_POSITION) {
						float transX = sample_valP->val.three_d.x - layerWidth / 2;
						float transY = -sample_valP->val.three_d.y + layerHeight / 2;	// AE的原点(0, 0)在左上角，meishe SDK的原点在画面中心位置
...
A_Time				currT)
{
	...
	if (compFile) {
		for (A_long iL = 0; !err && iL < num_layersL; iL++) {
			...
			fprintf(compFile, "<track source=\":1\" width=\"%d\" height=\"%d\" clipStart=\"%d\" clipDuration=\"%d\" blendingMode=\"%s\">\n",
			...
			DumpEffects(compFile, layerH, streamH);

			fprintf(compFile, "</track>\n");
		}
		fprintf(compFile, "\n\n");
		fclose(compFile);
	}
